# Google OAuth Setup for WHID

This guide walks you through creating the `credentials.json` file needed to connect WHID to your Gmail.
...
- Click **New Project**
- Name it something like `WHID` or `WhatHaveIDone`
- Click **Create**
...
- Click **Create**
- Fill in:
  - **App name**: `WHID` (or anything you want)
...
- Move it to your WHID project folder:

```bash
mv ~/Downloads/client_secret_*.json /path/to/WhatHaveIDone/credentials.json
```

### 7. Run WHID

```bash
cd /path/to/WhatHaveIDone
whid collect gmail
```

A browser window opens. Sign in with the Google account you want to export, and click **Allow**.

That's it. WHID will start downloading your emails.
...
3. Click **Advanced** > **Go to WHID (unsafe)** (it's your own app, this is safe)
...
6. Back in the terminal, WHID starts downloading
...
- **WHID uses `gmail.readonly` scope** — it can read your email but cannot modify, send, or delete anything
...
- All data stays on your machine. WHID never sends your emails anywhere
...
### "Access blocked: WHID has not completed the Google verification process"
...
Your token expired. Delete `token.json` and run `whid collect gmail` again to re-authenticate:
...
2. Kill any running WHID processes
3. Run `whid collect gmail` again
...
Another process is using the port WHID needs for OAuth. Close other running WHID instances or restart your terminal.
...
Your progress is saved — WHID picks up where it left off.
...
Free up disk space. WHID writes plain text JSONL files — a typical inbox (50k emails) takes about 1-2 GB.

## Revoking access

To disconnect WHID from your Google account:

1. Go to [myaccount.google.com/permissions](https://myaccount.google.com/permissions)
2. Find `WHID` in the list
3. Click **Remove Access**
4. Delete `token.json` from your WHID project folder
